...
import { verifyResetOtpRequest } from "../../api/api";

const C = { bg: '#0F172A', card: '#1E293B', primary: '#3B82F6', text: '#F1F5F9', muted: '#94A3B8', border: '#334155', error: '#EF4444' };
...
function ForgetPin() {
    ...
    const handleVerifyPin = async (e) => {
        ...
        if (!email) {
            setError("Session expired. Please request a reset code again.");
            return;
        }
        ...
        setIsLoading(true);
        try {
            await verifyResetOtpRequest({ email, otp: pin });
    ...
    return (
        <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem' }}>
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: '16px', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', width: '100%', maxWidth: '420px', padding: '2.5rem' }}>
                ...
                {error && (
                    <div style={{ background: 'rgba(239,68,68,0.1)', color: C.error, padding: '0.8rem', borderRadius: '10px', marginBottom: '1.25rem', fontSize: '0.85rem', textAlign: 'center', fontWeight: '500' }}>
                        {error}
                    </div>
                )}
                ...
                <form onSubmit={handleVerifyPin} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    <div>
                        ...
                        <input type="text" inputMode="numeric" required maxLength={6} value={pin} onChange={e => setPin(e.target.value.replace(/[^0-9]/g, ''))} placeholder="000000"
                            style={{ width: '100%', background: C.bg, border: `1.5px solid ${error ? C.error : C.border}`, borderRadius: '10px', padding: '0.9rem', fontSize: '1.5rem', color: C.text, outline: 'none', fontFamily: 'inherit', textAlign: 'center', letterSpacing: '0.5em', fontWeight: '700', transition: 'all 0.2s ease' }}
                    ...
                    <button type="submit" className="btn-primary" style={{ width: '100%', padding: '0.75rem' }} disabled={isLoading}>
