...
export const sendEmail = async ({ to, subject, html, useInterviewEmail = false }) => {
  // 🔥 Hardcoding verified email to avoid any Env mismatch
  const fromEmail = "mirzahaseeb0566@gmail.com";
 ...
 */
export const sendApplicationReceivedEmail = async ({ to, name, jobTitle, branchName, hrEmail }) => {
  const html = baseLayout(`
    ...
    <br/>
    <p style="color: #555;">Best regards,<br/>HR Team ${hrEmail ? `(${hrEmail})` : ""}</p>
  `);
 ...
 */
export const sendShortlistedEmail = async ({ to, name, jobTitle, hrEmail }) => {
  const html = baseLayout(`
 ...
 */
export const sendRejectedEmail = async ({ to, name, jobTitle, hrEmail }) => {
  const html = baseLayout(`
    <h2 style="color: #333;">Hi ${name},</h2>
 ...
 */
export const sendAcceptedEmail = async ({ to, name, jobTitle, branchName, hrEmail }) => {
  const html = baseLayout(`
 ...
 */
export const sendInterviewScheduledEmail = async ({ to, name, jobTitle, date, time, message, type, meetingLink, hrEmail }) => {
  ...
  const html = baseLayout(`
    ...
    <p>Please make sure to be available at the scheduled time. If you have any questions, contact your HR representative ${hrEmail ? `at <strong>${hrEmail}</strong>` : ""}.</p>
    <br/>
